...
class Assemble(Step):
    ...
    def get_vj_annot(self, df, chains, pairs):
    ...
    def gen_result(self):
        ...
        sum_dict = pd.read_csv(
            f'{self.outs}/metrics_summary.csv', sep=',', index_col=None)
        ...
        # write table
        title = 'Clonetypes'
        table_dict = self.get_table_dict(title, 'clonetypes_table', raw_clonotypes[[
        ...
        all_clonotypes.to_csv(
            f'{self.all_outs}/clonotypes.csv', sep=',', index=False)
        ...
        filter_contig.to_csv(
            f'{self.all_outs}/filtered_contig_annotations.csv', sep=',', index=False)
        ...
        if self.seqtype == "TCR":
            ...
            for c in self.chains:
                mid = filter_contig[filter_contig['chain']
                                    == c]['umis'].median()
                if mid == mid:
                    self.add_metric(
        ...
        self.get_vj_annot(filter_contig, self.chains, self.pair)
        ...
        self.add_data(chart=get_plot_elements.plot_barcode_rank(
            f'{self.all_outs}/count.txt'))
        ...
        self.get_vj_annot(df_match, self.chains, self.pair)

        # gen match results
        # df_match['contig_id'] = df_match['contig_id'].apply(lambda x: reversed_compl(barcode_dict[x.split('-')[0]]))
        df_match.to_csv(
            f'{self.match_outs}/match_contigs.csv', sep=',', index=False)
        ...
        df_match_clonetypes = pd.read_csv(
            f'{self.match_outs}/match_clonotypes.csv', sep='\t', index_col=None)
        ...
        df_match_clonetypes.to_csv(
            f'{self.match_outs}/match_clonotypes.csv', sep=',', index=False)
