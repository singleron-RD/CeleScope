...
class Otsu:
    ...
    def __init__(
        self, array, log_base=10, otsu_min_len=50, otsu_plot_path=None, **kwargs
    ):
        self.len_bool = True
    ...
    def _array2hist(self, binWidth=0.2):
        self.counts, self.bins = np.histogram(
            self.array, bins=np.arange(0, max(self.array) + binWidth, binWidth)
        )
...
class Auto:
    ...
    def run(self):
        ...
        count_cell_percentile = np.percentile(
            sorted_counts[:expected_cell_num], self.percentile
        )
