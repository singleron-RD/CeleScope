...
def find_nonambient_barcodes(
    ...
    print(
        "\n".join(
            list(map(lambda x: "{}: {}".format(*x), list(gg_filtered_metrics.items())))
        )
    )
    print("==============================")
    ...
    if len(eval_bcs) == 0:
        print(
            "Warning: no eval bcs are selected to evaluate non-empty bcs from SGT results!"
        )
        ...
        print(
            "Range candidate bc umis: {}, {}".format(
                umis_per_bc[eval_bcs].min(), umis_per_bc[eval_bcs].max()
            )
        )
        ...
        # Runxi's filtering
        print(
            "Identify {} cell-associated barcodes".format(
                len(orig_cells) + len(eval_bcs[is_nonambient])
            )
        )
