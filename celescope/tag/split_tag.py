...
def get_clonotypes_table(df):
    ...
    for c in chains:
        ...
        df_c = df_c.rename(
            columns={"aaSeqCDR3": f"{c}_aaSeqCDR3", "nSeqCDR3": f"{c}_nSeqCDR3"}
        )
    ...
    clonotypes["percent"] = clonotypes["barcode_count"].apply(
        lambda x: round(x / sum_cb, 2)
    )
...
class Split_tag(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        self.tag_barcode_dict = {
            tag: set(row["barcode"].tolist()) for tag, row in df_umi_tag.groupby("tag")
        }
        ...
        if args.split_vdj:
            self.cell_confident_vdj = glob.glob(
                f"{args.vdj_dir}/*count_vdj/*cell_confident.tsv*"
            )[0]
        ...
        if args.split_fl_vdj:
            ...
            if os.path.exists(f"{args.vdj_dir}/02.convert"):
                ...
                try:  # old version
                    ...
                    self.fasta_file = glob.glob(f"{match_out_dir}/match_contig.fasta")[
                        0
                    ]
                except IndexError:  # latest version
                    self.anno_file = glob.glob(
                        f"{match_out_dir}/matched_contig_annotations.csv"
                    )[0]
                    self.fasta_file = glob.glob(
                        f"{match_out_dir}/matched_contig.fasta"
                    )[0]

            # flv_trust4
            else:
                self.anno_file = glob.glob(
                    f"{args.vdj_dir}/04.summarize/*_filtered_contig.csv"
                )[0]
                self.fasta_file = glob.glob(
                    f"{args.vdj_dir}/04.summarize/*_filtered_contig.fasta"
                )[0]
    ...
    def split_matrix(self):
        for tag in self.tag_barcode_dict:
            ...
            tag_barcodes_indices = [
                raw_barcodes.index(barcode) for barcode in tag_barcodes
            ]
            tag_barcodes_indices.sort()
            slice_matrix = self.count_matrix.slice_matrix(tag_barcodes_indices)

            tag_matrix_dir = (
                f"{self.matrix_outdir}/{tag}_{FILTERED_MATRIX_DIR_SUFFIX[0]}/"
            )
    ...
    def split_bam(self):
        ...
        with pysam.AlignmentFile(self.args.bam_file, "r") as bam_in:
            ...
            for tag in tags:
                ...
                bam_handle_dict[tag] = pysam.AlignmentFile(
                    bam_file, "wb", header=bam_in.header
                )
    ...
    def split_vdj(self):
        ...
        for tag in self.tag_barcode_dict:
            ...
            if not temp.empty:
                ...
                temp.to_csv(
                    f"{self.vdj_outdir}/{tag}_{seqtype}_cell_confident.tsv",
                    sep="\t",
                    index=False,
                )
                clonotypes.to_csv(
                    f"{self.vdj_outdir}/{tag}_{seqtype}_clonotypes.tsv",
                    sep="\t",
                    index=False,
                )
            else:
    ...
    def split_fl_vdj(self):
        ...
        for tag in self.tag_barcode_dict:
            ...
            if not df_temp.empty:
                df_temp.to_csv(
                    f"{self.fl_vdj_outdir}/{tag}_{seqtype}_contig_annotations.csv",
                    sep=",",
                    index=False,
                )
                fasta_temp = open(
                    f"{self.fl_vdj_outdir}/{tag}_{seqtype}_contig.fasta", "w"
                )
                ...
                if "clonotype_id" in df_temp.columns:
                    df_temp = df_temp[
                        df_temp.clonotype_id != ""
                    ]  # del clonotype for trust4
                split_clonotypes = (
                    f"{self.fl_vdj_outdir}/{tag}_{seqtype}_clonotypes.csv"
                )
                ...
                utils.dump_dict_to_json(
                    metrics_dict, f"{self.fl_vdj_outdir}/{tag}_metrics.json"
                )
...
def get_opts_split_tag(parser, sub_program):
    ...
    parser.add_argument(
        "--vdj_dir",
        help="Match celescope vdj directory. Required when --split_vdj or --split_fl_vdj is specified.",
    )
    if sub_program:
        parser.add_argument("--umi_tag_file", help="UMI tag file.", required=True)
        parser.add_argument("--match_dir", help=HELP_DICT["match_dir"])
        parser.add_argument(
            "--matrix_dir", help="Match celescope scRNA-Seq matrix directory."
        )
