...
class Count_vdj(Step):
    ...
    def __init__(self, args, display_title=None):
        ...
        if utils.check_arg_not_none(self.args, "match_dir"):
            self.match_cell_barcodes, _match_cell_number = (
                utils.get_barcode_from_match_dir(args.match_dir)
            )
        ...
        self.df_UMI_sum = df_UMI_count_filter.groupby(["barcode"], as_index=False).agg(
            {"UMI": "sum"}
        )
        self.df_match_UMI_count_filter = df_UMI_count_filter[
            df_UMI_count_filter["barcode"].isin(self.match_cell_barcodes)
        ]
        self.df_match_UMI_sum = self.df_UMI_sum[
            self.df_UMI_sum["barcode"].isin(self.match_cell_barcodes)
        ]
    ...
    def cell_calling(self):
        ...
        df_cell = self.df_match_UMI_count_filter[
            self.df_match_UMI_count_filter["barcode"].isin(target_cell_barcodes)
        ]
        self.df_UMI_sum["mark"] = self.df_UMI_sum["barcode"].apply(
            lambda x: "CB" if x in target_cell_barcodes else "UB"
        )
    ...
    def get_df_confident(self, df_cell):
        ...
        df_confident = df_confident.groupby(["barcode", "chain"], as_index=False).head(
            1
        )
        return df_confident
    ...
    def get_clonetypes_and_write(self, df_valid_count):
        ...
        df_clonetypes["percent"] = (
            df_clonetypes.barcode / total_CDR3_barcode_number * 100
        )
    ...
    def add_metrics(self, df_valid_count, cell_barcodes):
        ...
        if self.args.type == "BCR":
            ...
            multi_chain_num = df_tmp[
                (df_tmp["aaSeqCDR3_IGK"] != "NA") | (df_tmp["aaSeqCDR3_IGL"] != "NA")
            ].shape[0]
            three_chain_num = df_tmp[
                (df_tmp["aaSeqCDR3_IGK"] != "NA") & (df_tmp["aaSeqCDR3_IGL"] != "NA")
            ].shape[0]
            self.add_metric(
    ...
    def write_cell_confident_count(self, df_valid_count, df_clonetypes, df_confident):
        df_mergeID = pd.merge(df_valid_count, df_clonetypes, how="left", on=self.cols)
    ...
    def write_clonetypes_table_to_data(self, df_clonetypes):
        ...
        def format_table(df_clonetypes):
            ...
            df_table["percent"] = df_table["percent"].apply(lambda x: str(x) + "%")
            seqs = ["aaSeqCDR3"]
            cols = []
            for chain in self.chains:
                for seq in seqs:
        ...
        table_dict = self.get_table_dict(
            title=title, table_id="clonetypes", df_table=df_table
        )
    ...
    def run(self):
        ...
        df_clonetypes = self.get_clonetypes_and_write(df_valid_count)
