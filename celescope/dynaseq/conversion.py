...
class Conversion(Step):
    ...
    def snp_candidate(self, df_arr):
        ...
        # all conv sites
        self.conv_df = Outputdf.groupby(['chrom', 'pos']).agg({"convs": "sum", "covers": "sum"})
    ...
    def add_conversion_metrics(self):
        ...
        self.add_metric(
            ...
            help_info=(f"The total number of SNP sites under the specified conditions: "
...
def get_opts_conversion(parser, sub_program):
    ...
    parser.add_argument(
        "--snp_threshold", default=0.5, type=float, help="snp threshold filter, greater than snp_threshold will be recognized as snp"
    )
    parser.add_argument(
        "--readsplit", default=1000000, type=int, help="split to have approximately N reads per output file"
    )
    parser.add_argument(
        "--conversionMem",
        default=5,
    ...
    if sub_program:
        ...
        parser.add_argument("--total_num", help="total number of input bam", type=int, required=False)
        parser.add_argument("--star_log", help="star log for extracting the total number (alternative to `total_num`)", required=False)
