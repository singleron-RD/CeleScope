...
class Multi_fusion(Multi):
    ...
    def count_fusion(self, sample):
        ...
        cmd_line = self.get_cmd_line(step, sample)
        capture_bam = (
            f'{self.outdir_dic[sample]["star_fusion"]}/{sample}_{STAR_BAM_SUFFIX}'
        )
        cmd = (
            f"{cmd_line} "
    ...
    def filter_fusion(self, sample):
        ...
        cmd_line = self.get_cmd_line(step, sample)
        raw_read_count_file = (
            f'{self.outdir_dic[sample]["count_fusion"]}/{sample}_raw_read_count.json'
        )
        cmd = (
            f"{cmd_line} "
            f"--match_dir {self.col4_dict[sample]} "
    ...
    def analysis_fusion(self, sample):
        ...
        cmd_line = self.get_cmd_line(step, sample)
        filter_umi_file = (
            f'{self.outdir_dic[sample]["filter_fusion"]}/{sample}_filtered_UMI.csv'
        )
        cmd = (
            f"{cmd_line} "
            f"--match_dir {self.col4_dict[sample]} "
