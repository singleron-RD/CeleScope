...
class Plot_vid:
    def __init__(self, sample, outdir, vid_list, snp_dir, match_dir):
        ...
        self.df_vid_tsne = pd.read_csv(
            vid_tsne_file, sep="\t", converters={"VID": ast.literal_eval}
        )
    ...
    def plot_vid(self):
        ...
        plot = ggplot(
            self.df_match_tsne, aes(x="tSNE_1", y="tSNE_2", color="VIDs")
        ) + geom_point(size=0.2)
...
def main():
    parser = argparse.ArgumentParser(
        description="plot snp", formatter_class=ArgFormatter
    )
    parser.add_argument(
        "--mapfile", help="mapfile with VIDs as 5th column", required=True
    )
