...
class Convert(Step):
    def __init__(self, args, display_title=None):
        ...
        whitelist_files = Chemistry.get_whitelist('rna_5p3p')
        self.barcode_set_list, self.barcode_mismatch_list = Bc.parse_whitelist_file(whitelist_files,
            n_pattern=len(self.pattern_dict_5p['C']), n_mismatch=1)
...
def get_opts_convert(parser, sub_program=True):
    if sub_program:
        parser.add_argument('--fq1_3p', help='3 prime R1 fastq file. Multiple files are separated by comma.', required=True)
        parser.add_argument('--fq2_3p', help='3 prime R2 fastq file. Multiple files are separated by comma.', required=True)
        parser.add_argument('--fq1_5p', help='5 prime R1 fastq file. Multiple files are separated by comma.', required=True)
        parser.add_argument('--fq2_5p', help='5 prime R2 fastq file. Multiple files are separated by comma.', required=True)
        parser.add_argument(
            '--stdout',
