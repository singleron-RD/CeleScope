...
class Multi_rna_virus(Multi):
    ...
    def count_virus(self, sample):
        step = "count_virus"
        barcode_file = (
            f'{self.outdir_dic[sample]["count"]}/{sample}_matrix_10X/barcodes.tsv'
        )
        virus_bam = f'{self.outdir_dic[sample]["star_virus"]}/{sample}_virus_Aligned.sortedByCoord.out.bam'
        cmd_line = self.get_cmd_line(step, sample)
        cmd = (
            f"{cmd_line} " f"--virus_bam {virus_bam} " f"--barcode_file {barcode_file} "
        )
    ...
    def analysis_rna_virus(self, sample):
        ...
        virus_file = (
            f'{self.outdir_dic[sample]["count_virus"]}/{sample}_virus_UMI_count.tsv'
        )
        matrix_file = f'{self.outdir_dic[sample]["count"]}/{sample}_matrix_10X'
        cmd_line = self.get_cmd_line(step, sample)
        cmd = (
            f"{cmd_line} " f"--virus_file {virus_file} " f"--matrix_file {matrix_file} "
        )
