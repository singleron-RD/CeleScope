...
class Mapping_vdj(super_vdj.Mapping_vdj):
    ...
    def mapping_summary(self):
        ...
        self.add_metric(
            name="Reads Mapped to Any VDJ Gene",
            value=df.shape[0],
            total=total_reads,
            help_info="Reads Mapped to any germline VDJ gene segments"
        )
        ...
        self.add_metric(
            ...
            total=total_reads,
            help_info="Reads with CDR3 sequence"
        )

        # Reads with Correct CDR3
        df_correct_cdr3 = df_cdr3[~(df_cdr3["cdr3_aa"].str.contains(r"\*")) & ~(df_cdr3["cdr3_aa"].str.contains("X"))]
        self.add_metric(
            name="Reads with Correct CDR3",
            value=df_correct_cdr3.shape[0],
            total=total_reads,
            help_info="Reads with CDR3 might have stop codon and these Reads are classified as incorrect"
        )
        
        # Reads Mapped Confidently To VJ Gene
        df_confident = df_correct_cdr3[df_correct_cdr3["productive"]=="T"]
        self.add_metric(
            name="Reads Mapped Confidently to VJ Gene",
            value=df_confident.shape[0],
            total=total_reads,
            help_info="Reads with productive rearrangement mapped to VJ gene pairs and with correct CDR3"
        )
        ...
        for chain in self.chains:
            ...
            self.add_metric(
                ...
                total=total_reads,
                help_info=f"Reads mapped confidently to {chain}"
            )
