...
def get_10X_barcode(cellranger_path):
    BARCODE_10X_FILE = (
        os.path.dirname(cellranger_path)
...
def run_convert(BARCODES_10X_FILE, fq2, outdir):
    ...
    for entry in fq_file:
        ...
        seq = entry.sequence
        qual = entry.quality
        new_seq1, new_qual1, new_seq2_1, new_qual2_1, new_seq2_2, new_qual2_2 = (
            convert_seq(barcode, umi, barcode_dict, barcodes_10X, seq, qual)
        )
    ...
    barcode_record.to_csv(
        f"{outdir}/barcode_correspondence_file", sep="\t", index=False
    )
...
def main():
    ...
    parser.add_argument(
        "--sgr_fq", help="read2 fastq file in 01.barcode", required=True
    )
    parser.add_argument(
        "--outdir", help="output path of 10X format fastq file", required=True
    )
