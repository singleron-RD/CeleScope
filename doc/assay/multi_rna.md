...
`--mt_gene_list` Specifies the path to a plain text file containing mitochondrial genes for the reference genome, with one gene symbol per line. During the `celescope utils  mkgtf` stage, a default `mt_gene_list.txt` is automatically generated by identifying all gene symbols that begin with prefix "MT-" or "mt-".
